    ...
    if (!LocationService.instance.isSupported) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Geolocalização não suportada neste dispositivo'),
  ...
  Future<void> _searchAddress() async {
    ...
    if (!LocationService.instance.isSupported) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Geocoding não suportado neste dispositivo'),
            backgroundColor: Colors.orange,
          ),
        );
      }
    ...
    try {
      final position = await LocationService.instance
          .getLocationFromAddress(_addressController.text.trim());
